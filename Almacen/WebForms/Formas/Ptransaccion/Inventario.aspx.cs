...
namespace Almacen.WebForms.Formas.Ptransaccion
{
    public partial class Inventario : BasePage
    {
        ...
        private void CargarCombos()
        {
            ...
            try
            {
                this.ddlAño.DataSource = periodo.PeriodoAñoAbierto(Convert.ToInt16(Session["empresa"]));
        ...
        private void cargarGrupo()
        {
            try
            {
                ...
                dvPlanes.RowFilter = "empresa = " + this.Session["empresa"].ToString() + " and activo = True";
                dvPlanes.Sort = "descripcion";
                this.ddlGrupo.DataSource = dvPlanes;
        ...
        private void cargarSubGrupo()
        {
            try
            {
                ...
                DataView dvPlanes = CentidadMetodos.EntidadGet("iMayorItem", "ppa").Tables[0].DefaultView;
                dvPlanes.RowFilter = "empresa = " + this.Session["empresa"].ToString() + " and activo = True";
                dvPlanes.Sort = "descripcion";
                this.ddlSubgrupo.DataSource = dvPlanes;
                this.ddlSubgrupo.DataValueField = "codigo";
                this.ddlSubgrupo.DataTextField = "descripcion";
                this.ddlSubgrupo.DataBind();
                this.ddlSubgrupo.Items.Insert(0, new ListItem("", ""));
            }
            catch (Exception ex)
            {
                ManejoErrorCatch(ex);
        ...
        protected void lbPreLiquidar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ddlSubgrupo.SelectedValue.Trim().Length == 0 & ddlSubgrupo.Visible == true)
                {
