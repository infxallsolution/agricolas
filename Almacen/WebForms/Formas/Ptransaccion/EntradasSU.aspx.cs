...
{
    public partial class EntradasSU : BasePage
    {
        #region Instancias
        //--------------------------------------------------------------------------
        ...
        private void inhabilitarItemsOC()
        {
            foreach (GridViewRow gvr in gvLista.Rows)
            {
                ...
                EnumerableRowCollection<DataRow> query = from tipodoc in dvTipoDocto.Table.AsEnumerable()
                                                         where tipodoc.Field<int>("empresa") == Convert.ToInt16(Session["empresa"]) & tipodoc.Field<bool>("suministro") == true
                                                         select tipodoc;
        ...
        private void cargarBodega()
        {
            try
            {
                ...
                dvBodega.RowFilter = "productoTerminado='True' and empresa=" + Session["empresa"].ToString();
        ...
        private void CargarTipoTransaccion()
        {
            try
            {
                this.ddlTipoDocumento.DataSource = tipoTransaccion.GetTipoTransaccionModuloForma(Convert.ToInt16(Session["empresa"]), 21);
