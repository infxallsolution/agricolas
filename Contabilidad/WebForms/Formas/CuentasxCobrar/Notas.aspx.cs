...
namespace Contabilidad.WebForms.Formas.CuentasxCobrar
{
    public partial class Notas : BasePage
    {
        ...
        private void CargarTipoTransaccion()
        {
            try
            {
                this.ddlTipoDocumento.DataSource = tipoTransaccion.GetTipoTransaccionModuloForma(Convert.ToInt16(Session["empresa"]), 29);
        ...
        private void CargaProductos()
        {
            try
            {
                ...
                if (ddlDocRef.SelectedValue.Trim().Length > 0)
                {
                    DataView dvProducto = item.RetornaItemsFiltroDocReferencia(Convert.ToInt32(this.Session["empresa"]), "", ddlDocRef.SelectedValue);
        ...
        protected void ddlTipoDocumento_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                lblDocRef.Visible = true;
                ddlDocRef.Enabled = true;
                btnRefrescarDocRef.Visible = true;
                btnRefrescarDocRef.Enabled = true;
        ...
        protected void txtFiltroProducto_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ddlDocRef.SelectedValue.Trim().Length == 0)
                {
                    ManejoError("Debe seleccionar un documento de referencia", "I");
                    txtFiltroProducto.Focus();
                    return;
                }

                DataView dvitems = item.RetornaItemsFiltroDocReferencia(Convert.ToInt32(this.Session["empresa"]), txtFiltroProducto.Text, ddlDocRef.SelectedValue);
                if (dvitems.Count == 0)
                {
        ...
        protected void imbRefrescarDoc_Click(object sender, EventArgs e)
        {
            CargarDocReferencia();
        }
        ...
        private void CargarDocReferencia()
        {
            if (ddlTercero.SelectedValue.Length > 0 && ddlSucursal.SelectedValue.Length > 0)
            {
                try
                {
                    this.ddlDocRef.DataSource = tipoTransaccion.SeleccionaDocuemntoReferenciaFacturasTercero(Convert.ToInt16(Session["empresa"]), ddlTercero.SelectedValue, ddlSucursal.SelectedValue);
                    this.ddlDocRef.DataValueField = "numero";
                    this.ddlDocRef.DataTextField = "cadena";
                    this.ddlDocRef.DataBind();
                    this.ddlDocRef.Items.Insert(0, new ListItem("", ""));
                    this.ddlDocRef.SelectedValue = "";
                }
                catch (Exception ex)
                {
                    ManejoError("Error al cargar tipos de transacción. Correspondiente a: " + ex.Message, "C");
        ...
        protected void btnRefrescarDocRef_Click(object sender, EventArgs e)
        {
            CargarDocReferencia();
        }
    }
