...
{
    public partial class GrupoProveedores : BasePage
    {

        #region Instancias
        Security seguridad = new Security();
        CIP ip = new CIP();
        CgrupoProveedor grupooProveedor = new CgrupoProveedor();
        ...
        private void EntidadKey()
        {
            ...
            try
            {
                if (CentidadMetodos.EntidadGetKey("cxpGrupoProveedor", "ppa", objKey).Tables[0].Rows.Count > 0)
                {
                    this.nilblInformacion.Visible = true;
        ...
        private void Guardar()
        {
            ...
            try
            {
                ...
                Convert.ToInt16(Session["empresa"])
                    ...
                    switch (CentidadMetodos.EntidadInsertUpdateDelete("cxpGrupoProveedor", operacion, "ppa", objValores))
                    {
                        case 0:
                            if (Convert.ToBoolean(this.Session["editar"]) == true)
                            {
                                for (int x = 0; x < this.selFuncionarios.Items.Count; x++)
                                {
                                    if (grupooProveedor.VerificaProveedorGrupo(txtCodigo.Text, Convert.ToString(this.selFuncionarios.Items[x].Value), Convert.ToInt16(Session["empresa"])) == 0)
                                    {
                                        if (this.selFuncionarios.Items[x].Selected == false)
                                        {

                                            object[] objValoresConcepto = new object[] { Convert.ToInt16(Session["empresa"]), Convert.ToString(txtCodigo.Text), Convert.ToInt16(this.selFuncionarios.Items[x].Value) };

                                            switch (CentidadMetodos.EntidadInsertUpdateDelete("cxpGrupoProveedordetalle", "elimina", "ppa", objValoresConcepto))
                                            {
                                                case 1:
                                                    verificacion = true;
                                                    break;
                                    ...
                                    else
                                    {
                                        if (this.selFuncionarios.Items[x].Selected == true)
                                        {
                                            ...
                                            switch (CentidadMetodos.EntidadInsertUpdateDelete("cxpGrupoProveedordetalle", "inserta", "ppa", objValoresConcepto))
                                            {
                                                case 1:
                                                    verificacion = true;
                                                    break;
                            ...
                            else
                            {
                                for (int x = 0; x < this.selFuncionarios.Items.Count; x++)
                                {
                                    if (this.selFuncionarios.Items[x].Selected == true)
                                    {
                                        if (grupooProveedor.VerificaProveedorGrupo(txtCodigo.Text, Convert.ToString(this.selFuncionarios.Items[x].Value), Convert.ToInt16(Session["empresa"])) == 1)
                                        {

                                            object[] objValoresConcepto = new object[] { Convert.ToInt16(Session["empresa"]), Convert.ToString(txtCodigo.Text), Convert.ToInt16(this.selFuncionarios.Items[x].Value) };

                                            switch (CentidadMetodos.EntidadInsertUpdateDelete("cxpGrupoProveedordetalle", operacion, "ppa", objValoresConcepto))
                                            {
                                                case 1:
                                                    verificacion = true;
                                                    break;
        ...
        private void ValidaRegistro()
        {
            for (int x = 0; x < this.selFuncionarios.Items.Count; x++)
            {
                if (grupooProveedor.VerificaProveedorGrupo(txtCodigo.Text, Convert.ToString(selFuncionarios.Items[x].Value), Convert.ToInt16(Session["empresa"])) == 0)
        ...
        protected void gvLista_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (CentidadMetodos.EntidadInsertUpdateDelete("cxpGrupoProveedor", operacion, "ppa", objValores) == 0)
