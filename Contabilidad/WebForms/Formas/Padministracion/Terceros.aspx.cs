...
namespace Contabilidad.WebForms.Formas.Padministracion
{
    public partial class Terceros : BasePage
    {
        ...
        private void CargarCombos()
        {
            ...
            try
            {
                var data = CentidadMetodos.EntidadGet("gPais", "ppa").Tables[0].AsEnumerable()
                    .Where(x => x.Field<int>("empresa") == Convert.ToInt32(this.Session["empresa"]))
                    .Select(x => new { codigo = x.Field<string>("codigo"), descripcion = x.Field<string>("codigo") + "-" + x.Field<string>("descripcion") }).ToList();
                this.ddlPais.DataSource = data;
        ...
        private void cargarCiudad(string departamento)
        {
            try
            {
                var ciudad = CentidadMetodos.EntidadGet("gCiudad", "ppa").Tables[0]
                    .AsEnumerable().Where(x => x.Field<int>("empresa") == Convert.ToInt32(this.Session["empresa"])
                    && x.Field<string>("departamento").Trim() == departamento.Trim() &&
                     x.Field<string>("pais").Trim() == ddlPais.SelectedValue.Trim()
                    ).Select(x => new { codigo = x.Field<string>("codigo"), descripcion = x.Field<string>("codigo") + "-" + x.Field<string>("descripcion") }).ToList();
        ...
        private void cargarDepartamento(string pais)
        {
            try
            {
                var ciudad = CentidadMetodos.EntidadGet("gDepartamento", "ppa").Tables[0].AsEnumerable()
                    .Where(x => x.Field<int>("empresa") == Convert.ToInt32(this.Session["empresa"])
                    && x.Field<string>("pais").Trim() == pais.Trim()
                    ).Select(x => new { codigo = x.Field<string>("codigo"), descripcion = x.Field<string>("codigo") + "-" + x.Field<string>("descripcion") }).ToList();
        ...
        protected void gvLista_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                fila += 1;
                chkCliente.Checked = Convert.ToBoolean(dvTercero.Table.Rows[0].ItemArray.GetValue(fila));
                fila += 1;
                chkProveedor.Checked = Convert.ToBoolean(dvTercero.Table.Rows[0].ItemArray.GetValue(fila));
                fila += 1;
                chkEmpleado.Checked = Convert.ToBoolean(dvTercero.Table.Rows[0].ItemArray.GetValue(fila));
                fila += 1;
                chkAccionista.Checked = Convert.ToBoolean(dvTercero.Table.Rows[0].ItemArray.GetValue(fila));
                fila += 1;
                chkContratista.Checked = Convert.ToBoolean(dvTercero.Table.Rows[0].ItemArray.GetValue(fila));
                fila += 1;
                chkExtractora.Checked = Convert.ToBoolean(dvTercero.Table.Rows[0].ItemArray.GetValue(fila));
                fila += 1;
                chkComercializadora.Checked = Convert.ToBoolean(dvTercero.Table.Rows[0].ItemArray.GetValue(fila));
                fila += 2;
                if (dvTercero.Table.Rows[0].ItemArray.GetValue(fila) != null)
        ...
        protected void lbRegistrar_Click(object sender, EventArgs e)
        {
            ...
            if (rbTipoPersona.SelectedIndex == 1 & txtApellido1.Visible)
            {
            ...
            if (rbTipoPersona.SelectedIndex == 2)
            {
                ...
                if (txtDocumento.Text.Trim().Length == 0 || txtRazonSocial.Text.Trim().Length == 0)
                {
                    this.nilblMensaje.Text = "Ingrese el documento y la razon social";
                    return;
                }
            }
