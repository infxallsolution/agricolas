...
{
    public partial class Ciudad : BasePage
    {
        #region Instancias
        ...
        private void GetEntidad()
        {
            try
            {
                ...
                if (Convert.ToString(this.niddlDeparamento.SelectedValue.Trim()).Length == 0)
                {
                    ManejoError("Debe seleccionar un país para realizar la busqueda", "I");
                ...
                {
                    this.gvLista.DataSource = ciudad.BuscarEntidad(this.nitxtBusqueda.Text, Convert.ToString(this.niddlDeparamento.SelectedValue),
        ...
        private void CargarCombos()
        {
            try
            {
                this.niddlDeparamento.DataSource = CcontrolesUsuario.OrdenarEntidadSinEmpresa(CentidadMetodos.EntidadGet("gDepartamento", "ppa"), "descripcion");
                this.niddlDeparamento.DataValueField = "codigo";
                this.niddlDeparamento.DataTextField = "descripcion";
                this.niddlDeparamento.DataBind();
                this.niddlDeparamento.Items.Insert(0, new System.Web.UI.WebControls.ListItem("", ""));
            }
            catch (Exception ex)
            {
                ManejoErrorCatch(ex);
            ...
            try
            {
                this.ddlPais.DataSource = CcontrolesUsuario.OrdenarEntidadSinEmpresa(CentidadMetodos.EntidadGet("gPais", "ppa"), "descripcion");
                this.ddlPais.DataValueField = "codigo";
                this.ddlPais.DataTextField = "descripcion";
                this.ddlPais.DataBind();
                this.ddlPais.Items.Insert(0, new System.Web.UI.WebControls.ListItem("", ""));
            }catch(Exception ex)
            {
                ManejoErrorCatch(ex);
            }
        ...
        {
            object[] objKey = new object[] { this.txtCodigo.Text, niddlDeparamento.SelectedValue, Convert.ToInt16(Session["empresa"]) };

            try
            {
                if (Convert.ToString(this.niddlDeparamento.SelectedValue.Trim()).Length == 0)
                {
                    ManejoError("Debe seleccionar una país para realizar la busqueda", "I");
                    this.gvLista.DataSource = null;
                    this.gvLista.DataBind();
                    this.nilblInformacion.Visible = true;
                    this.nilblInformacion.Text = this.gvLista.Rows.Count.ToString() + " Registros encontrados";
                ...
                if (CentidadMetodos.EntidadGetKey("gCiudad", "ppa", objKey).Tables[0].Rows.Count > 0)
                {
                    ManejoError(Convert.ToString(this.niddlDeparamento.SelectedValue) + " " + this.txtCodigo.Text + " ya se encuentra registrado", "I");
        ...
        private void Guardar()
        {
            ...
            try
            {
                ...
                this.txtCodigo.Text,                        //varchar
                niddlDeparamento.SelectedValue,             //varchar
                this.txtDescripcion.Text,                   //varchar
                Convert.ToInt32(this.Session["empresa"]),   //int
                ddlPais.SelectedValue,                      //char

            };

                switch (CentidadMetodos.EntidadInsertUpdateDelete("gCiudad", operacion, "ppa", objValores))
                {
                    case 0:
        ...
        protected void gvLista_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            try
            {
                ...
                object[] objValores = new object[] { Convert.ToString(this.gvLista.Rows[e.RowIndex].Cells[2].Text), niddlDeparamento.SelectedValue, Convert.ToInt16(this.Session["empresa"]) };

                if (CentidadMetodos.EntidadInsertUpdateDelete("gCiudad", operacion, "ppa", objValores) == 0)
